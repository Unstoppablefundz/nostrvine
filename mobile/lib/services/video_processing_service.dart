...
class VideoProcessingService {
  ...
  Future<VideoProcessingResult> pollForProcessingCompletion({
    ...
    Log.info('🔄 Starting video processing poll for $videoId',
        name: 'VideoProcessingService', category: LogCategory.video);

    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        Log.info('📡 Poll attempt $attempt/$maxAttempts for $fileHash',
            name: 'VideoProcessingService', category: LogCategory.video);
        ...
        if (response.statusCode == 200) {
          ...
          if (blobData is Map) {
            ...
            Log.info('✅ Video processing complete: $mediaUrl',
                name: 'VideoProcessingService', category: LogCategory.video);
            Log.info('  HLS: $hlsUrl', name: 'VideoProcessingService', category: LogCategory.video);
            Log.info('  Thumbnail: $thumbnailUrl', name: 'VideoProcessingService', category: LogCategory.video);
            ...
            Log.error('❌ Invalid blob descriptor format from Blossom server',
                name: 'VideoProcessingService', category: LogCategory.video);
            return VideoProcessingResult(
              success: false,
          ...
          Log.info('⏳ Video still processing (HTTP 202), attempt $attempt/$maxAttempts',
              name: 'VideoProcessingService', category: LogCategory.video);
          ...
          Log.error('❌ Unexpected response: ${response.statusCode} - ${response.data}',
              name: 'VideoProcessingService', category: LogCategory.video);
          return VideoProcessingResult(
            success: false,
            errorMessage: 'Processing failed: ${response.statusCode} - ${response.data}',
          );
        }
      } on DioException catch (e) {
        Log.error('🌐 Network error during polling attempt $attempt: ${e.message}',
            name: 'VideoProcessingService', category: LogCategory.video);

        if (attempt == maxAttempts) {
          return VideoProcessingResult(
            success: false,
            errorMessage: 'Network error after $maxAttempts attempts: ${e.message}',
          );
        }
        ...
        Log.error('💥 Unexpected error during polling attempt $attempt: $e',
            name: 'VideoProcessingService', category: LogCategory.video);
        return VideoProcessingResult(
          success: false,
    ...
    Log.error('⏱️ Video processing timeout after $maxAttempts attempts',
        name: 'VideoProcessingService', category: LogCategory.video);
    return VideoProcessingResult(
      success: false,
