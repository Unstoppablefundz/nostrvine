...
void main() {
  group('VideoProcessingService', () {
    ...
    test('should poll blob descriptor until HTTP 200 and return processing result', () async {
      const serverUrl = 'https://cf-stream-service-prod.protestnet.workers.dev';
      const fileHash = 'fa54a5b814fe5b25c512025e83dd7acf8b40a75537080956a4f0ed6069f645fc';
      const videoId = 'bbdfb69ae2be4acda66edc3a8b8ef66a';

      // Mock sequence: 202 -> 202 -> 200 (processing -> processing -> ready)
      var callCount = 0;
      when(mockDio.get('$serverUrl/$fileHash', options: anyNamed('options')))
          .thenAnswer((_) async {
            callCount++;
            if (callCount <= 2) {
              return Response(
              ...
              return Response(
                ...
                data: {
                  ...
                  'thumbnail': 'https://stream.cloudflare.com/$videoId/thumbnail.jpg',
                },
                requestOptions: RequestOptions(path: ''),
              );
            }
      ...
      expect(result.hlsUrl, 'https://stream.cloudflare.com/$videoId/manifest.m3u8');
      expect(result.thumbnailUrl, 'https://stream.cloudflare.com/$videoId/thumbnail.jpg');

      // Should have polled 3 times (202, 202, 200)
      verify(mockDio.get('$serverUrl/$fileHash', options: anyNamed('options'))).called(3);
    });
