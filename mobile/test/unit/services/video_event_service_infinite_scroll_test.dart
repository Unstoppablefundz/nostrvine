...
void main() {
  ...
  group('Infinite Scroll with Until Filter', () {
    test('should use until filter when loading more events at end of feed', () async {
      ...
      final paginationStates = videoEventService.getPaginationStatesForTesting();
      final discoveryState = paginationStates[SubscriptionType.discovery]!;
      expect(discoveryState.oldestTimestamp, equals(1704060000)); // Should be automatically set
    ...
    test('should not use until filter when no existing events', () async {
      ...
      final paginationStates = videoEventService.getPaginationStatesForTesting();
      final discoveryState = paginationStates[SubscriptionType.discovery]!;
      expect(discoveryState.oldestTimestamp, isNull); // Should be null initially
      ...
      expect(capturedFilter, isNotNull);
      expect(capturedFilter!.until, isNull); // No until filter when no existing events
      expect(capturedFilter!.limit, equals(50));
      expect(capturedFilter!.kinds, contains(32222));
    ...
    test('should properly update pagination state when receiving older events', () async {
      ...
      // Assert
      expect(discoveryState.oldestTimestamp, equals(1704056400)); // Should be oldest event
      expect(discoveryState.eventsReceivedInCurrentQuery, equals(3));
      expect(discoveryState.hasMore, isTrue); // Got exactly what we requested, so hasMore = true
      
      // Cleanup
      await streamController.close();
    });

    test('should set hasMore=false when fewer events received than requested', () async {
      ...
      expect(discoveryState.hasMore, isFalse); // Should be false since we got less than requested
      
      // Cleanup
      await streamController.close();
    });

    test('should continue loading older events with decreasing until timestamps', () async {
      ...
      expect(capturedFilters[1].until, equals(1704067200)); // Second load uses first timestamp
      expect(capturedFilters[2].until, equals(1704060000)); // Third load uses second timestamp
...
Event _createMockNostrEvent(String id, int createdAt) {
  ...
  final validPubkey = '1234567890abcdef' * 4; // Repeat 16 chars 4 times = 64 chars
